...
async function getIpApi(ip: string): Promise<{
  ...
  try {
    ...
    const ccFlag = ccCode ? String.fromCodePoint(...ccCode.toUpperCase().split('').map((c: string) => 127397 + c.charCodeAt(0))) : '';
  ...
  cmdHandler: async (msg: Api.Message) => {
    ...
    try {
      if (command === 'list') {
        ...
        if (servers.length === 0) {
          await msg.edit({ text: "❌ <b>错误</b>\n\n无可用服务器", parseMode: "html" });
          return;
        }
        
        const serverList = servers.slice(0, 20).map(server => 
          `<code>${server.id}</code> - <code>${htmlEscape(server.name)}</code> - <code>${htmlEscape(server.location)}</code>`
        ).join('\n');
        ...
        if (!serverId || isNaN(serverId)) {
          await msg.edit({ text: "❌ <b>参数错误</b>\n\n请指定有效的服务器ID\n例: <code>s set 12345</code>", parseMode: "html" });
          return;
        }
        ...
        await msg.edit({ text: "🔄 正在更新 Speedtest CLI...", parseMode: "html" });
        ...
        try {
          ...
          await msg.edit({
            text: `<blockquote><b>⚡️SPEEDTEST by OOKLA</b></blockquote>\n<code>更新失败: ${htmlEscape(String(error))}</code>`,
            parseMode: "html"
          });
        }
        ...
        try {
          ...
          const description = [
            ...
            `<code>Node</code>  <code>${result.server.id}</code> - <code>${htmlEscape(result.server.name)}</code> - <code>${htmlEscape(result.server.location)}</code>`,
            `<code>Conn</code>  <code>${result.interface.externalIp.includes(':') ? 'IPv6' : 'IPv4'}</code> - <code>${htmlEscape(result.interface.name)}</code> - <code>MTU</code> <code>${mtu}</code>`,
            `<code>Ping</code>  <code>⇔${result.ping.latency}ms</code> <code>±${result.ping.jitter}ms</code>`,
            `<code>Rate</code>  <code>↓${await unitConvert(result.download.bandwidth)}</code> <code>↑${await unitConvert(result.upload.bandwidth)}</code>`,
            `<code>Data</code>  <code>↓${await unitConvert(result.download.bytes, true)}</code> <code>↑${await unitConvert(result.upload.bytes, true)}</code>`,
            `<code>Stat</code>  <code>RX ${await unitConvert(rxBytes, true)}</code> <code>TX ${await unitConvert(txBytes, true)}</code>`,
            `<code>Time</code>  <code>${result.timestamp.replace('T', ' ').split('.')[0].replace('Z', '')}</code>`
          ].join('\n');
          ...
          await msg.edit({
            text: `❌ <b>速度测试失败</b>\n\n<code>${htmlEscape(String(error))}</code>`,
            parseMode: "html"
          });
        }
