...
const parseBackgroundColor = (args: string[]): string => {
  ...
  if (param in CONFIG.THEME_COLORS) {
    ...
    return CONFIG.THEME_COLORS[param as keyof typeof CONFIG.THEME_COLORS] || 'transparent';
  }
...
const buildQuotePayload = async (message: Api.Message, backgroundColor: string, client: any): Promise<QuotePayload> => {
  try {
...
const generateQuoteImage = async (payload: QuotePayload): Promise<Buffer | null> => {
  try {
...
• 必须回复要引用的消息才能使用`,
  ...
  try {
    ...
    const payload = await buildQuotePayload(targetMessage, backgroundColor, client);
