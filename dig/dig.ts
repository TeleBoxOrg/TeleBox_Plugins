...
const COMMON_RECORD_TYPES = ['A', 'AAAA', 'MX', 'CNAME', 'TXT', 'NS', 'SOA', 'PTR', 'SRV', 'CAA'];
...
async function getIpLocation(ip: string): Promise<string> {
  try {
    ...
    if (response.status === 200 && response.data) {
      ...
      if (region && region !== country) location += (location ? '-' : '') + region;
    ...
    try {
      ...
      if (response.status === 200 && response.data && !response.data.bogon) {
        ...
        if (region && region !== city) location += (location ? ',' : '') + region;
      ...
      console.error('Both ip.sb and ipinfo.io APIs failed for IP location lookup');
    }
  }
...
async function executeDig(args: string[]): Promise<any> {
  try {
    ...
    const isSimpleQuery = !hasAnyPlusOption && !modifiedArgs.some(arg => arg.startsWith('-'));
    ...
    if (error.message?.includes('not found') || error.message?.includes('not recognized')) {
      throw new Error('系统未安装 dig 工具。请安装 bind-utils (Linux) 或 dnsutils (Ubuntu/Debian) 包。');
    }
...
async function formatShortDnsResults(output: string, args: string[]): Promise<string> {
...
function parseDnsRecord(line: string): { domain: string; type: string; value: string; ttl?: string } | null {
...
async function formatDetailedDnsResults(output: string, args: string[]): Promise<string> {
  if (!output || output.trim() === '') {
    return `❌ <b>解析失败</b>
...
无记录或查询失败`;
  ...
  if (answerSection.length === 0) {
    const contentLines = lines.filter(line => {
      ...
      return trimmed !== '' && 
             !trimmed.startsWith(';;') && 
             !trimmed.startsWith(';') &&
             trimmed.includes('\t') || trimmed.split(/\s+/).length >= 4;
    });
  ...
  for (let i = 0; i < displayLines.length; i++) {
    ...
    if (record) {
      ...
      resultText += `\n\n${icon} <b>${record.type}</b>: <code>${htmlEscape(record.value)}</code>`;
  ...
  if (hasMore) {
    resultText += `\n\n<i>... 还有 ${answerSection.length - 3} 个结果未显示</i>`;
  }
  
  return resultText;
}

const digPlugin: Plugin = {
  command: ["dig"],
  description: `
调用系统 dig 命令进行查询并显示 IP 归属地及 ASN
  `,
