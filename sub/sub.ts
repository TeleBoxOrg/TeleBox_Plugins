          ...
          case "status":
            ...
            try {
              ...
              if (fs.existsSync(secretFile)) {
                ...
                const containerStatus = await sh("docker ps --format '{{.Names}} {{.Status}}' | grep sub-store || echo '未运行'");
                const ip = await sh("curl -s --max-time 3 ifconfig.me").catch(() => "未知");
...
echo "后端: http://\$IP:3001/\$SECRET"`;
          ...
          case "status":
            const status = await sh("docker ps --format '{{.Names}} {{.Status}}' | grep sub-store || echo '未运行'");
          ...
          case "logs":
            ...
            try {
              ...
              const todayLogs = await sh(`docker logs sub-store --since ${today}T00:00:00 2>&1 || echo '今日无日志'`);
              ...
              if (client && me) {
                ...
                await msg.edit({ text: `✅ 今日日志已发送至收藏夹\n文件大小: ${(size/1024).toFixed(1)}KB` });
              } else {
