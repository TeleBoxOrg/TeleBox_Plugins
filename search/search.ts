...
class SearchService {
        ...
        for (const channelHandle of channels) {
            try {
                ...
                if (!(entity instanceof Api.Channel) && !(entity instanceof Api.Chat)) {
                    await msg.edit({ text: `错误： ${channelHandle.trim()} 不是公开频道或群组。` });
                    continue;
                }
                ...
                this.config.channelList.push({ id: channelId, title: entity.title, handle: channelHandle.trim() });
    ...
    private async handleDelete(msg: Api.Message, args: string) {
        if (!args) throw new Error("用法: .so del <频道链接>。使用 \\ 分隔多个频道。");
        const channels = args.split('\\');
        let removedCount = 0;
        ...
        for (const channelHandle of channels) {
            try {
                ...
                this.config.channelList = this.config.channelList.filter(c => c.id !== channelId);

                if (this.config.channelList.length === initialLength) {
                    await msg.edit({ text: `❓ 目标 "${(entity as any).title}" 不在列表中。` });
                    continue;
                }

                if (this.config.defaultChannel === channelId) {
                    this.config.defaultChannel = this.config.channelList.length > 0 ? this.config.channelList[0].id : null;
                }
                removedCount++;
            } catch (error: any) {
                await msg.edit({ text: `删除频道 ${channelHandle.trim()} 时出错： ${error.message}` });
            }
        }
    ...
    private async handleDefault(msg: Api.Message, args: string) {
        if (!args) throw new Error("用法: .so default <频道链接> 或 .so default d 删除默认频道。");
        if (args === 'd') {
    ...
    private async handleList(msg: Api.Message) {
        ...
        const searchOrderIds = [...new Set([this.config.defaultChannel, ...this.config.channelList.map(c => c.id)].filter(Boolean))];
        searchOrderIds.forEach((id, index) => {
            const channel = this.config.channelList.find(c => c.id === id);
            if (channel) {
                const isDefault = channel.id === this.config.defaultChannel ? " (默认)" : "";
    ...
    private async handleExport(msg: Api.Message) {
        ...
        const backupContent = this.config.channelList.map(c => c.handle || `https://t.me/c/${c.id.replace('-100', '')}`).join('\n');
        const tempDir = path.join(process.cwd(), "temp");
        await fs.mkdir(tempDir, { recursive: true });
        const backupFilePath = path.join(tempDir, "so_channels_backup.txt");
        await fs.writeFile(backupFilePath, backupContent);
        await this.client.sendFile(msg.chatId!, { file: backupFilePath, caption: `✅ 您的频道源已导出。\n回复此文件并发送 \`.so import\` 即可恢复。`, replyTo: msg });
    ...
    private async handleImport(msg: Api.Message) {
        ...
        if (!buffer || buffer.length === 0) throw new Error("下载文件失败或文件为空。");

        const handles = buffer.toString().split('\n').map((h: string) => h.trim()).filter(Boolean);
        if (handles.length === 0) throw new Error("备份文件中没有有效的频道。");

        await msg.edit({ text: `⚙️ 正在清除旧配置并重新添加 ${handles.length} 个源...` });
        ...
        for (const handle of handles) {
            try {
                ...
                if ((entity instanceof Api.Channel || entity instanceof Api.Chat) && !newConfig.channelList.some(c => c.id === entity.id.toString())) {
                    const channelId = entity.id.toString();
                    newConfig.channelList.push({ id: channelId, title: entity.title, handle: handle });
        ...
        await this.saveConfig();
        await msg.edit({ text: `✅ 恢复成功：已导入 ${successCount}/${handles.length} 个频道源。` });
    }

    private async handleKkp(msg: Api.Message, useSpoiler: boolean, useForceDownload: boolean) {
        await this.findAndSendVideo(msg, null, useSpoiler, useForceDownload, "kkp");
    }

    private async handleSearch(msg: Api.Message, query: string, useSpoiler: boolean, useForceDownload: boolean) {
        if (!query) throw new Error("请输入搜索关键词。");
        await this.findAndSendVideo(msg, query, useSpoiler, useForceDownload, "search");
    }

    private async findAndSendVideo(msg: Api.Message, query: string | null, useSpoiler: boolean, useForceDownload: boolean, type: "kkp" | "search") {
        if (this.config.channelList.length === 0) throw new Error("请至少使用 `.so add` 添加一个搜索频道。");
        await msg.edit({ text: type === 'kkp' ? '🎲 正在随机寻找视频...' : '🔍 正在搜索视频...' });
        const searchOrder = [...new Set([this.config.defaultChannel, ...this.config.channelList.map(c => c.id)].filter(Boolean) as string[])];
        let validVideos: Api.Message[] = [];
        let allVideosForFallback: Api.Message[] = [];
        for (const channelId of searchOrder) {
            const channelInfo = this.config.channelList.find(c => c.id === channelId);
            if (!channelInfo) continue;
            try {
                await msg.edit({ text: `- 正在搜索... (源: ${searchOrder.indexOf(channelId) + 1}/${searchOrder.length})` });
                const channelEntity = await this.client.getEntity(channelInfo.id);
                const videos = await this.client.getMessages(utils.getInputPeer(channelEntity), {
                    limit: 100,
                    filter: new Api.InputMessagesFilterVideo()
                });
                if (type === 'search') allVideosForFallback.push(...videos);
                validVideos.push(...videos.filter((v: Api.Message) => {
                    const isPureVideo = v.video && !(v.media instanceof Api.MessageMediaWebPage) && !(v.entities && v.entities.some(e => e instanceof Api.MessageEntityUrl || e instanceof Api.MessageEntityTextUrl));
                    if (type === 'kkp') {
                        const durationAttr = v.video?.attributes.find(a => a instanceof Api.DocumentAttributeVideo) as Api.DocumentAttributeVideo | undefined;
                        return isPureVideo && durationAttr && durationAttr.duration !== undefined && durationAttr.duration <= 60;
                    }
                    return isPureVideo && this.isMessageMatching(v, query!);
                }));
            } catch (e: any) {
                if (e instanceof Error && e.message.includes("Could not find the input entity")) {
                    console.error("无法找到频道实体，尝试刷新对话缓存...");
                    await this.client.getDialogs();
                    try {
                        await this.client.getEntity(channelInfo.id);
                    } catch (refreshError: unknown) {
                        console.error(`刷新后仍无法找到频道实体: ${refreshError instanceof Error ? refreshError.message : refreshError}`);
                        continue;
                    }
                } else {
                    console.error(`在频道 "${channelInfo.title}" (${channelId}) 中失败: ${e instanceof Error ? e.message : e}`);
                    continue;
                }
            }
        }
        if (validVideos.length === 0) {
            if (type === 'search' && allVideosForFallback.length > 0) {
                ...
                await this.sendVideo(msg, this.selectRandomVideo(allVideosForFallback), useSpoiler, useForceDownload, `[无匹配] ${query}`);
                return;
            }
            await msg.edit({ text: type === 'kkp' ? '🤷‍♂️ 未找到合适的视频。' : '❌ 在任何频道中均未找到结果。' });
            return;
        }
        await this.sendVideo(msg, this.selectRandomVideo(validVideos), useSpoiler, useForceDownload, query);
    }

    private async sendVideo(originalMsg: Api.Message, video: Api.Message, useSpoiler: boolean, forceDownload: boolean, caption?: string | null) {
        ...
        const sendAsDownloaded = async () => {
            ...
            try {
                ...
                if (useSpoiler) {
                    ...
                    const inputFile = await this.client.uploadFile({ file: fileToUpload, workers: 1 });

                    const videoAttr = video.video.attributes.find((attr): attr is Api.DocumentAttributeVideo => attr instanceof Api.DocumentAttributeVideo);
                    ...
                    await this.client.sendFile(originalMsg.chatId!, { file: tempFilePath, caption: caption || video.text || undefined });
                }
            } finally {
                try {
        ...
        if (useSpoiler || forceDownload) {
            ...
            try {
                ...
                if (error.message && error.message.includes('CHAT_FORWARDS_RESTRICTED')) {
                    await originalMsg.edit({ text: `⚠️ 转发失败，频道限制。正在切换到下载模式...` });
                    await sendAsDownloaded();
                } else {
                    throw error;
                }
    ...
    private isMessageMatching(message: Api.Message, query: string): boolean {
        ...
        if (fileNameAttr && fileNameAttr.fileName.toLowerCase().includes(lowerQuery)) {
            return true;
        }
...
const ChannelSearchPlugin: Plugin = {
    command: ["so"],
    description: `强大的多频道资源搜索插件，具备高级功能：
...
- 优化的模糊匹配算法`,
    ...
    cmdHandler: async (msg: Api.Message): Promise<void> => {
...
export default ChannelSearchPlugin;
