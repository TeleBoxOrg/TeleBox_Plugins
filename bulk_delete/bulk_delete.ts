 ...
 */
const bulkDeletePlugin: Plugin = {
  command: ["bd"],
  description:
    "回复消息并使用 .bd, 删除从被回复的消息到当前指令之间的所有消息。或使用 .bd <数字> 删除您最近的消息。使用 .bd on/off 切换删除他人消息的权限。",
  ...
  cmdHandler: async (msg: Api.Message) => {
    ...
    if (!msg.replyTo) {
      ...
      // B. 如果只是 .bd
      const currentMode =
        userDeleteMode.get(userId) === false ? "关闭" : "开启";
...
export default bulkDeletePlugin;
