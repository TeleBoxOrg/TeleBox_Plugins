...
async function searchAndSendMusic(keyword, client, chatId) {
  try {
    // 获取bot实体
    ...
    for (const msg of messages) {
      if (msg.text && msg.text.includes(keyword)) {
        ...
        if (msg.replyMarkup && msg.replyMarkup.rows) {
          ...
          if (firstButton && firstButton.data) {
            // 获取bot实体
            const botEntity = await client.getEntity("Music163bot");
            ...
            // 获取音频消息
            const audioMessages = await client.getMessages("Music163bot", {
              limit: 3,
            });
...
async function sendMusicById(songId, client, chatId) {
  try {
    // 获取bot实体
    const botEntity = await client.getEntity("Music163bot");
...
const neteasePlugin: Plugin = {
  ...
  cmdHandler: async (msg) => {
    ...
    try {
      ...
      if (/^\d+$/.test(args.trim())) {
        // 纯数字ID
        await sendMusicById(args.trim(), msg.client, msg.peerId);
      } else if (args.includes("music.163.com")) {
        // 网易云链接
        const songId = extractSongId(args);
        if (songId) {
          await sendMusicById(songId, msg.client, msg.peerId);
        } else {
        ...
        // 歌曲搜索
        await searchAndSendMusic(args, msg.client, msg.peerId);
      }

      await msg.delete();
    } catch (error) {
      await msg.edit({ text: `获取音频失败: ${error?.message || "未知错误"}` });
    }
  },
};
