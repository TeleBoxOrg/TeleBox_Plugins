...
function getParticipants(): number[] {
  ...
  const getStmt = db.prepare("SELECT value FROM lottery_state WHERE key = ?");
  const result = getStmt.get("lottery.participants") as { value: string } | undefined;
...
function addParticipant(userId: number): number {
  ...
  if (!participants.includes(userId)) {
    ...
    const setStmt = db.prepare("INSERT OR REPLACE INTO lottery_state (key, value) VALUES (?, ?)");
...
function clearLotteryData(): void {
  ...
  const keys = [
    "lottery.start", "lottery.participants", "lottery.chat_id",
    "lottery.num", "lottery.win", "lottery.title", "lottery.keyword"
  ];
    ...
    for (const uid of winUsers) {
      try {
        ...
        if (userEntity && "id" in userEntity) {
          // 获取完整用户信息
          const fullInfo = await client.invoke(new Api.users.GetFullUser({
            id: userEntity
          }));
...
用户发送关键词即可参与，达到人数自动开奖
  `,
  listenMessageHandler: handleLotteryJoin,
